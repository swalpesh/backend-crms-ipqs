...
function isAssociateHead(user) {
  return (
    user?.department_id === "Associate-Marketing" &&
    user?.role_id === "Associate-Marketing-Head"
  );
}

function isAssociateEmployee(user) {
  return (
    user?.department_id === "Associate-Marketing" &&
    user?.role_id === "Associate-Marketing-Employee"
  );
}
...
export const getScheduledAssociateVisits = async (req, res) => {
  try {
    ...
    let query = `
      SELECT 
        l.*,
        DATE_FORMAT(l.associate_visit_date, '%Y-%m-%d') AS associate_visit_date,
        CONCAT(assignee.first_name, ' ', assignee.last_name) AS assigned_employee_name,
        assignee.username AS assigned_employee_username,
        CONCAT(creator.first_name, ' ', creator.last_name) AS created_by_name,
        creator.username AS created_by_username
    ...
    console.error("Error fetching scheduled associate visits:", error);
...
export const AssociateTeamTodaysVisits = async (req, res) => {
  try {
    ...
    const [leads] = await pool.query(
      `SELECT * FROM leads 
       WHERE lead_stage = 'Associate-Marketing'
