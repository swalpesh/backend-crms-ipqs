...
// 📝 CHANGED: Function name updated to isSilverlineAssociateHead
function isSilverlineAssociateHead(user) {
  return (
    // 📝 CHANGED: department_id check updated to 'Silverline-Associates'
    user?.department_id === "Silverline-Associates" &&
    // 📝 CHANGED: role_id check updated to 'Silverline-Associates-Head'
    user?.role_id === "Silverline-Associates-Head"
  );
}

// 📝 CHANGED: Function name updated to isSilverlineAssociateEmployee
function isSilverlineAssociateEmployee(user) {
  return (
    // 📝 CHANGED: department_id check updated to 'Silverline-Associates'
    user?.department_id === "Silverline-Associates" &&
    // 📝 CHANGED: role_id check updated to 'Silverline-Associates-Employee'
    user?.role_id === "Silverline-Associates-Employee"
  );
}
  ...
  try {
    ...
    await pool.query(
      ...
      [
        ...
        // 📝 CHANGED: Default lead_stage mapped to "Silverline-Associates"
        lead_stage || "Silverline-Associates",
    ...
    await pool.query(
      ...
      // 📝 CHANGED: Backup log default mapped to "Silverline-Associates"
      [lead_id, lead_stage || "Silverline-Associates", assigned_employee || "0"]
    );
...
export const listLeadsByEmployee = async (req, res) => {
  try {
    ...
    let query = `
      ...
      -- 📝 CHANGED: Filter by 'Silverline-Associates' stage
      AND l.lead_stage = 'Silverline-Associates'
    `;
  ...
  try {
    ...
    const [leads] = await pool.query(
      `SELECT * FROM leads 
       ...
       -- 📝 CHANGED: Filter by 'Silverline-Associates' stage
       AND lead_stage = 'Silverline-Associates'
...
// 📝 CHANGED: Function name changed to SilverlineAssociatesAllLeads
export const SilverlineAssociatesAllLeads = async (req, res) => {
  try {
    const [employees] = await pool.query(
      // 📝 CHANGED: Query updated to fetch 'Silverline-Associates' employees
      "SELECT employee_id, username, email, role_id FROM employees WHERE department_id = 'Silverline-Associates'"
    );
    ...
    for (const emp of employees) {
      const [leads] = await pool.query(
        // 📝 CHANGED: Query updated to fetch 'Silverline-Associates' leads for employee
        "SELECT * FROM leads WHERE assigned_employee = ? AND lead_stage = 'Silverline-Associates'",
        [emp.employee_id]
      );
    ...
    const [unassigned] = await pool.query(
      // 📝 CHANGED: Query updated to fetch unassigned 'Silverline-Associates' leads
      "SELECT * FROM leads WHERE assigned_employee = '0' AND lead_stage = 'Silverline-Associates'"
    );
    ...
    res.status(200).json({
      // 📝 CHANGED: Response message and department tag updated to 'Silverline-Associates'
      message: "Silverline Associates employees and their leads fetched successfully",
      department: "Silverline-Associates",
      total_employees: data.employees.length,
      total_unassigned_leads: data.unassigned_leads.length,
      ...data,
    });
  } catch (error) {
    console.error("Error fetching Silverline Associates leads:", error);
    res.status(500).json({ error: "Server error" });
  }
};
...
export const changeLeadStageByIpqsHead = async (req, res) => {
  try {
    ...
    // 📝 CHANGED: Allowed roles updated for Silverline Associates
    const allowedRoles = [
      "IpqsHead",
      "Silverline-Associates-Head",
      "Silverline-Associates-Employee",
    ];
...
// 📝 CHANGED: Function name updated to assignLeadToSilverlineAssociateEmployee
export const assignLeadToSilverlineAssociateEmployee = async (req, res) => {
  try {
    // 📝 CHANGED: Destructured request body variables renamed for silverline_associate
    const {
      lead_id,
      assigned_employee,
      silverline_associate_visit_date,
      silverline_associate_visit_time,
      silverline_associate_visit_priority,
      silverline_associate_visit_type,
      reason,
    } = req.body;

    const headId = req.user.employee_id;
    // 📝 CHANGED: Department variable set to 'Silverline-Associates'
    const department = "Silverline-Associates";
    ...
    // 📝 CHANGED: UPDATE query columns changed to silverline_associate_*
    await pool.query(
      `
      UPDATE leads
      SET 
        ...
        silverline_associate_visit_priority = ?,
        silverline_associate_visit_type = ?,
        silverline_associate_visit_status = 'Pending',
        updated_at = NOW()
      WHERE lead_id = ?
      `,
      [
        ...
        silverline_associate_visit_date || null,
        silverline_associate_visit_time || null,
        silverline_associate_visit_priority || "Medium",
    ...
    await pool.query(
      ...
      [
        ...
        "silverline_associate_visit_scheduled", 
        // 📝 CHANGED: reason string updated
        reason || "Silverline Associates visit scheduled",
      ]
    );

    res.status(200).json({
      // 📝 CHANGED: Response message and payload keys updated
      message: "Silverline Associates visit scheduled successfully",
      lead_id,
      assigned_employee,
      silverline_associate_visit_date,
      silverline_associate_visit_time,
...
// 📝 CHANGED: Function name updated to getSilverlineAssociatesVisitDetails
export const getSilverlineAssociatesVisitDetails = async (req, res) => {
  try {
    const headId = req.user.employee_id;
    const roleId = req.user.role_id;

    // 📝 CHANGED: Role check updated to 'Silverline-Associates-Head'
    if (roleId !== "Silverline-Associates-Head") {
      return res.status(403).json({
        error: "Forbidden: Only Silverline Associates Head can access visit details.",
      });
    }

    // 📝 CHANGED: Query updated to select silverline_associate_* columns and filter by stage
    const query = `
      SELECT 
        l.lead_id,
        l.company_name, 
        l.lead_name, 
        l.silverline_associate_visit_date, 
        l.silverline_associate_visit_time, 
        l.silverline_associate_visit_priority, 
        l.assigned_employee,
        e.first_name,
        e.last_name,
        e.username
      ...
      WHERE l.lead_stage = 'Silverline-Associates'
      ORDER BY l.silverline_associate_visit_date DESC, l.silverline_associate_visit_time ASC
    `;
    ...
    for (const row of rows) {
      ...
      // 📝 CHANGED: Mapping silverline database columns to JSON object
      const visitData = {
        lead_id: row.lead_id,
        company_name: row.company_name,
        lead_name: row.lead_name,
        visit_date: row.silverline_associate_visit_date,
        visit_time: row.silverline_associate_visit_time,
        visit_priority: row.silverline_associate_visit_priority,
    ...
    return res.status(200).json({
      ...
      message: "Silverline Associates visit details fetched successfully",
...
// 📝 CHANGED: Function name updated to getUnscheduledSilverlineAssociateLeads
export const getUnscheduledSilverlineAssociateLeads = async (req, res) => {
  try {
    const employeeId = req.user.employee_id;

    // 📝 CHANGED: Query updated to check silverline_associate_* columns and stage
    const query = `
      SELECT 
        l.*,
        CONCAT(assignee.first_name, ' ', assignee.last_name) AS assigned_employee_name,
      ...
      LEFT JOIN employees creator 
        ON l.created_by COLLATE utf8mb4_unicode_ci = creator.employee_id COLLATE utf8mb4_unicode_ci
      WHERE l.lead_stage = 'Silverline-Associates'
        AND l.silverline_associate_visit_date IS NULL
        AND l.silverline_associate_visit_time IS NULL
    ...
    return res.status(200).json({
      // 📝 CHANGED: Response message updated
      message: "Unscheduled Silverline Associates leads fetched successfully",
      employee_id: employeeId,
...
// 📝 CHANGED: Function name updated to getScheduledSilverlineAssociateVisits
export const getScheduledSilverlineAssociateVisits = async (req, res) => {
  try {
    const employeeId = req.user.employee_id;
    const { date } = req.query; 

    // 📝 CHANGED: Query updated for silverline_associate_visit_date format and stage
    let query = `
      SELECT 
        l.*,
        DATE_FORMAT(l.silverline_associate_visit_date, '%Y-%m-%d') AS silverline_associate_visit_date,
        CONCAT(assignee.first_name, ' ', assignee.last_name) AS assigned_employee_name,
        assignee.username AS assigned_employee_username,
        CONCAT(creator.first_name, ' ', creator.last_name) AS created_by_name,
        creator.username AS created_by_username
      ...
      WHERE l.lead_stage = 'Silverline-Associates'
        AND l.assigned_employee = ?
        AND l.silverline_associate_visit_date IS NOT NULL
    `;
    ...
    if (date) {
      ...
      // 📝 CHANGED: Query updated to append silverline date filter
      query += ` AND l.silverline_associate_visit_date = ?`;
      params.push(date);
    }

    // 📝 CHANGED: Query ORDER BY updated to use silverline_associate columns
    query += ` ORDER BY l.silverline_associate_visit_date ASC, l.silverline_associate_visit_time ASC`;
...
// 📝 CHANGED: Function name updated to updateSilverlineAssociateVisitStatus
export const updateSilverlineAssociateVisitStatus = async (req, res) => {
  try {
    ...
    if (status === "Started") {
      ...
      // 📝 CHANGED: Database UPDATE columns mapped to silverline_associate_*
      query = `
        UPDATE leads 
        SET silverline_associate_lead_visit_status = ?, silverline_associate_visit_start_location = ?, updated_at = NOW() 
        WHERE lead_id = ?
      `;
      ...
      query = `
        UPDATE leads 
        SET silverline_associate_lead_visit_status = ?, updated_at = NOW() 
        WHERE lead_id = ?
      `;
    ...
    // 📝 CHANGED: changeType logic strings updated for Silverline Associate
    const changeType = status === "Started" ? "Silverline Associate Visit Started" :
                       status === "Completed" ? "Silverline Associate Visit Completed" :
                       `Silverline Associate Visit ${status}`;

    // 📝 CHANGED: reasonText logic strings updated for Silverline Associates
    const reasonText = status === "Started" 
      ? `Silverline Associates visit started at location: ${location}` 
      : `Silverline Associates visit status updated to ${status}`;
    ...
    await pool.query(
      ...
      [
        ...
        req.user.department_id || "Silverline-Associates",
        req.user.role_id,
        changeType,
        reasonText
      ]
...
/* ---------------- Reschedule Visit ---------------- */
// 📝 CHANGED: Function name updated to rescheduleSilverlineAssociateVisits
export const rescheduleSilverlineAssociateVisits = async (req, res) => {
  try {
    const { lead_id, new_visit_date, new_visit_time, reason } = req.body;
    const employeeId = req.user.employee_id;
    // 📝 CHANGED: Fallback department updated
    const departmentId = req.user.department_id || "Silverline-Associates";
    ...
    // 📝 CHANGED: Selected columns updated to silverline_associate_*
    const [existing] = await pool.query(
      "SELECT assigned_employee, lead_stage, silverline_associate_visit_date, silverline_associate_visit_time FROM leads WHERE lead_id = ?",
      [lead_id]
    );
    ...
    const oldDate = oldLead.silverline_associate_visit_date 
      ? new Date(oldLead.silverline_associate_visit_date).toISOString().split('T')[0] 
      : "Unscheduled";
    const oldTime = oldLead.silverline_associate_visit_time || "Unscheduled";

    // 📝 CHANGED: UPDATE query columns updated to silverline_associate_*
    await pool.query(
      `
      UPDATE leads 
      SET 
        silverline_associate_visit_date = ?, 
        silverline_associate_visit_time = ?, 
        silverline_associate_lead_visit_status = 'Pending', 
        updated_at = NOW() 
      WHERE lead_id = ?
      `,
    ...
    await pool.query(
      ...
      [
        ...
        "Silverline Associate Visit Rescheduled",
        reasonText.trim()
      ]
    );
    ...
    return res.status(200).json({
      // 📝 CHANGED: Response message updated
      message: "Silverline Associates visit rescheduled successfully.",
      lead_id,
      new_visit_date,
      new_visit_time,
...
/* ---------------- Get Completed Leads ---------------- */
// 📝 CHANGED: Function name updated to getCompletedSilverlineAssociateVisits
export const getCompletedSilverlineAssociateVisits = async (req, res) => {
  try {
    const roleId = req.user.role_id;
    const employeeId = req.user.employee_id;

    // 📝 CHANGED: Allowed roles updated
    const allowedRoles = ["Silverline-Associates-Head", "Silverline-Associates-Employee"];
    if (!allowedRoles.includes(roleId)) {
      return res.status(403).json({
        error: "Forbidden: Only Silverline Associates Team members can access this data.",
      });
    }
    ...
    // 📝 CHANGED: JOIN condition mapped to check for 'Silverline Associate Visit Completed'
    const baseJoins = `
      FROM leads l
      LEFT JOIN lead_activity_backup lab 
        ON l.lead_id = lab.lead_id AND lab.change_type = 'Silverline Associate Visit Completed'
      LEFT JOIN employees e 
    ...
    // 📝 CHANGED: Role check condition updated to 'Silverline-Associates-Head'
    if (roleId === "Silverline-Associates-Head") {
      query = `
        SELECT ${selectFields}
        ${baseJoins}
        -- 📝 CHANGED: WHERE condition checks silverline_associate_lead_visit_status
        WHERE l.silverline_associate_lead_visit_status = 'Completed'
    ...
    return res.status(200).json({
      // 📝 CHANGED: Response message updated
      message: "Completed Silverline Associates visits fetched successfully",
      // 📝 CHANGED: Role check in response object updated
      view_mode: roleId === "Silverline-Associates-Head" ? "All Team Data" : "Personal History",
      total: leads.length,
      leads,
    });
  } catch (error) {
...
// 📝 CHANGED: Function name updated to SilverlineAssociateTeamTodaysVisits
export const SilverlineAssociateTeamTodaysVisits = async (req, res) => {
  try {
    const roleId = req.user.role_id;

    // 📝 CHANGED: Role check updated to include 'Silverline-Associates-Head'
    if (!["Silverline-Associates-Head", "IpqsHead"].includes(roleId)) {
      return res.status(403).json({
        error: "Forbidden: Only Silverline Associates Head or IpqsHead can access this.",
      });
    }
    ...
    // 📝 CHANGED: Query updated to search for '%Silverline-Associates%' departments
    const [employees] = await pool.query(
      `SELECT employee_id, first_name, last_name, username, email, department_id 
       FROM employees 
       WHERE department_id LIKE '%Silverline-Associates%' AND status = 'active'`
    );
    
    // 📝 CHANGED: Query updated to check silverline_associate_visit_date and lead_stage
    const [leads] = await pool.query(
      `SELECT * FROM leads 
       WHERE lead_stage LIKE '%Silverline-Associates%' 
       AND silverline_associate_visit_date = ? 
       ORDER BY silverline_associate_visit_time ASC`,
      [targetDate]
    );
    ...
    res.status(200).json({
      ...
      message: `Silverline Associates visits for ${targetDate} fetched successfully`,
      date: targetDate, 
      accessed_by: roleId,
      department: "Silverline-Associates",
...
// 📝 CHANGED: Function name updated to getHotSilverlineAssociateLeads
export const getHotSilverlineAssociateLeads = async (req, res) => {
  try {
    const roleId = req.user.role_id;
    const employeeId = req.user.employee_id;

    // 📝 CHANGED: WHERE condition updated for 'Silverline-Associates' lead stage
    let query = `
      SELECT 
        l.*,
        CONCAT(assignee.first_name, ' ', assignee.last_name) AS assigned_employee_name,
    ...
    const headRoles = ["Silverline-Associates-Head", "IpqsHead"];
...
// 📝 CHANGED: Function name updated to getSilverlineAssociatesEmployeesRevenue
export const getSilverlineAssociatesEmployeesRevenue = async (req, res) => {
  try {
    const roleId = req.user.role_id;
    
    // 📝 CHANGED: Allowed roles updated
    const allowedRoles = ["Silverline-Associates-Head", "Silverline-Associates-Employee", "IpqsHead"];
    if (!allowedRoles.includes(roleId)) {
      return res.status(403).json({ 
    ...
    const empQuery = `
      ...
      FROM employees
      WHERE department_id = 'Silverline-Associates' AND status = 'active'
    `;
    const [employees] = await pool.query(empQuery);

    // 📝 CHANGED: Query updated to check silverline_associate_lead_visit_status and change_type
    const leadsQuery = `
      SELECT 
      ...
      WHERE lab.change_type = 'Silverline Associate Visit Completed'
        AND l.silverline_associate_lead_visit_status = 'Completed'
    `;
    ...
    return res.status(200).json({
      // 📝 CHANGED: Response message updated
      message: "Silverline Associates employees and detailed revenue fetched successfully",
...
export const getNewAssignedLeadsSummary = async (req, res) => {
  try {
    ...
    // 📝 CHANGED: Query updated for lead_stage 'Silverline-Associates'
    const leadsQuery = `
      SELECT 
        l.lead_id, 
        l.lead_name, 
...
export const getSalesFunnel = async (req, res) => {
  try {
    ...
    // 📝 CHANGED: Role check updated to check for 'Silverline-Associates-Head'
    const isHead = ["Silverline-Associates-Head", "IpqsHead"].includes(roleId);
    const params = isHead ? [] : [employeeId, employeeId];

    // 📝 CHANGED: Query updated to pull silverline_associate_* fields and check stage
    const funnelQuery = `
      SELECT 
        l.lead_id, 
        l.silverline_associate_visit_date, 
        l.silverline_associate_lead_visit_status, 
        l.lead_stage
      FROM leads l
      WHERE 
        (l.lead_stage = 'Silverline-Associates' ${isHead ? "" : "AND l.assigned_employee = ?"})
        OR 
        l.lead_id IN (
          SELECT lead_id 
          FROM lead_activity_backup 
          WHERE old_lead_stage = 'Silverline-Associates' 
            AND new_lead_stage != 'Silverline-Associates'
    ...
    funnelLeads.forEach((lead) => {
      // 📝 CHANGED: Accessing the specific date column mapped to Silverline
      if (lead.silverline_associate_visit_date) {
        scheduledVisits++;
      }
      // 📝 CHANGED: Accessing the specific status column mapped to Silverline
      if (lead.silverline_associate_lead_visit_status === 'Completed') {
        completedVisits++;
      }
      // 📝 CHANGED: Checking lead stage transfer logic
      if (lead.lead_stage !== 'Silverline-Associates') {
        transferredVisits++;
      }
    });
